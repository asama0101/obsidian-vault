...
- 人間が資料を投入するフォルダとしては`Documents/`だけを直下に置く。ドラッグ&ドロップで投入するゾーンであり、「人間は直接編集しない」と定義した`Cabinet/`の中にあるべきでないため
...
- `Cabinet/Diary/`フォルダをブックマークし、いつでもすぐ開けるようにする。デイリーノートはファイル名が日々変わるためファイル単位のピン留めは使えないが、フォルダのブックマークならそのまま効く
