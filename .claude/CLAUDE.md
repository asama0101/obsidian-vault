...
- `/today`: 秘書ループの開始または更新。現在のgitブランチと `Cabinet/Diary/<今日の日付>.md` の有無で判別する。**判別表の正典は `secretary` スキルの「モードの判別」節**であり、ここに書くのはその要約である。大枠は、`main` にいれば開始、当日の日次ブランチにいて当日のデイリーノートがあれば更新、当日以外の日次ブランチまたは想定外のブランチにいれば停止となる。中断や締めの失敗から復旧する組み合わせも表が定めているので、実際の判定は必ず表を見る。更新は `/loop` から30分間隔で自動的に呼ばれる。
