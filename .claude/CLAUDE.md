...
AIは人間が書いた文言を書き換えない。捕捉してノート化した処理済みのメモ行は削除せず、` → [[ノート名]]` を追記する。既存ノートへの操作指示として直接実行した行は ` → 実行済み（<何をしたか>）` になる。AIが `## メモ` に書く行は決まった接頭辞を持ち、処理対象にしない。接頭辞の実体と判定規約の詳細は `.claude/skills/secretary/SKILL.md` が正典。
