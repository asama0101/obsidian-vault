...
スラッシュコマンドは持たない。自然言語での指示が `today` スキルを起動する。

- 「今日のノートを開いて」等: 秘書ループの開始または更新。現在のgitブランチと `Cabinet/Diary/<今日の日付>.md` の有無で判別する。**判別表の正典は `today` スキルの「モードの判別」節**であり、ここに書くのはその要約である。大枠は、`main` にいれば開始、当日の日次ブランチにいて当日のデイリーノートがあれば更新、当日以外の日次ブランチまたは想定外のブランチにいれば停止となる。中断や締めの失敗から復旧する組み合わせも表が定めているので、実際の判定は必ず表を見る。更新は `/loop` から30分間隔で自動的に呼ばれる。
- 会話にメール・チャットの本文をそのまま貼り付ける: 取り込みモード。貼られた本文をノート化する。明示的な合図の言葉は不要。
- 「締めて」（または「クリアデスクして」）: 締めモード。1日を締める。時刻では自動起動しない。
- 「〇〇にタスク追加して」「棚卸しして」等: プロジェクト管理アクション。タスクの即時追加、またはproject/taskの棚卸しレポートを返す。詳細は `today` スキルの `references/project-management.md` を参照する。
