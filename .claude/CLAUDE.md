...
AIは人間が書いた文言を書き換えない。捕捉してノート化した処理済みのメモ行は削除せず、` → [[ノート名]]` を追記する。`context` の新しい値を提案したときだけ ` → [[ノート名]]（context: 提案値）` になる。既存ノートへの操作指示として直接実行した行は ` → 実行済み（<何をしたか>）` になる。AIが `## メモ` に書く行（`⚠`/`ⓘ`/`前日の未処理:` の接頭辞を持つ）は処理対象にしない。判定規約の詳細は `.claude/skills/secretary/SKILL.md` が正典。
