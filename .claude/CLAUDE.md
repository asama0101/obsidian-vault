...
- **人間がやること**: デイリーノートの `## メモ` に思いついたことや受けた依頼を書く。`## Claudeが確認したいこと` の `→` の右に回答を書く。1日の最初に「今日のノートを開いて」等とClaude Codeに伝える。
...
| `## Claudeが確認したいこと` | `- [ ] <質問>（N回目） →` | 未チェックで質問行を追加、回答済み行はチェック済み（`- [x]`）にして保持 | `→` の右に回答 |
...
AIは人間が書いた文言を書き換えない。捕捉してノート化した処理済みのメモ行は削除せず、` → [[ノート名]]` を追記する。既存ノートへの操作指示として直接実行した行は ` → 実行済み（<何をしたか>）` になる。AIが `## メモ` に書く行は決まった接頭辞を持ち、処理対象にしない。接頭辞の実体と判定規約の詳細は `.claude/skills/today/references/diary-note.md` が正典。
