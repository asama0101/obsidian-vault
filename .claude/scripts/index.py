...
`2_Notes/`・`4_Knowledge/`・`1_Inbox/`・`3_Archive/` の4フォルダを再帰的に走査する。
project ノート自体も `2_Notes/<案件名>/<案件名>.md` としてこの中に含まれるため、
vault直下は索引対象外である。秘書ループがノート一覧を読む唯一の経路。個別ノートの
読み込みは、実際にそのノートを編集するときだけ行う。出力はヘッダ行を持たず、
1行が1ノートに対応する。
"""
...
NOTE_DIRS = ["2_Notes", "4_Knowledge", "1_Inbox", "3_Archive"]


def collect(vault_root: Path) -> list[dict[str, str]]:
    """索引対象フォルダをパス順に索引化する。

    対象は `2_Notes/`・`4_Knowledge/`・`1_Inbox/`・`3_Archive/` の再帰走査のみ。
    project ノート自体も `2_Notes/<案件名>/<案件名>.md` としてこの中に含まれる。
    """
    notes: list[Path] = []
