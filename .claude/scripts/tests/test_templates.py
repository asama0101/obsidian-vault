...
EXPECTED_KEYS = {
    "task": ["type", "date", "status", "due", "done", "project", "blocked_by"],
...
EXPECTED_HEADINGS = {
    "task": ["## 進捗メモ"],
    "project": ["## 概要", "## マイルストーン", "## 連絡先・外部リンク", "## ドキュメント", "## タスク", "## 議事録", "## メモ"],
...
def test_today_template_has_the_five_sections():
...
def test_new_note_can_generate_every_type(vault, note_type):
    ...
    if note_type == "project":
        assert created_path == f"2_Notes/{title}/{title}.md"
    elif note_type == "know-how":
        assert created_path == f"4_Knowledge/{title}.md"
    else:
        assert created_path == f"1_Inbox/{title}.md"
