...
def test_columns_are_in_fixed_order(tmp_path):
    ...
    assert row[0] == "2_Notes/見積書の作成.md"
    assert row[1] == "task"
    assert row[2] == "1_todo"
...
def write_note_in(vault: Path, folder: str, name: str, body: str) -> Path:
    """一時vaultの 2_Notes/<folder>/ にノートを1件置く。"""
    notes = vault / "2_Notes" / folder
    notes.mkdir(parents=True, exist_ok=True)
    path = notes / name
    path.write_text(body, encoding="utf-8")
    return path
...
def test_path_column_includes_the_project_folder(tmp_path):
    ...
    assert result[0][0] == "2_Notes/大手町DC コアSW更改/C9500 見積依頼.md"


def test_know_how_folder_is_scanned(tmp_path):
    know_how = tmp_path / "4_Knowledge"
    know_how.mkdir(parents=True)
    (know_how / "BGPのルートリフレクタ設計.md").write_text("---\ntype: know-how\n---\n", encoding="utf-8")

    result = rows(run_index(tmp_path))

    assert result[0][0] == "4_Knowledge/BGPのルートリフレクタ設計.md"


def test_inbox_folder_is_scanned(tmp_path):
    inbox = tmp_path / "1_Inbox"
    inbox.mkdir(parents=True)
    (inbox / "プロジェクト管理スキルを作る.md").write_text("---\ntype: task\n---\n", encoding="utf-8")

    result = rows(run_index(tmp_path))

    assert result[0][0] == "1_Inbox/プロジェクト管理スキルを作る.md"


def test_archive_folder_is_scanned(tmp_path):
    archive = tmp_path / "3_Archive" / "旧案件"
    archive.mkdir(parents=True)
    (archive / "旧案件.md").write_text("---\ntype: project\nstatus: 2_done\n---\n", encoding="utf-8")

    result = rows(run_index(tmp_path, "--all"))

    assert result[0][0] == "3_Archive/旧案件/旧案件.md"


def test_project_note_under_its_own_2_notes_folder_is_listed(tmp_path):
    write_note_in(tmp_path, "奈良旅行", "奈良旅行.md", "---\ntype: project\nstatus: 1_active\n---\n")

    result = rows(run_index(tmp_path))

    assert result[0][0] == "2_Notes/奈良旅行/奈良旅行.md"
    assert result[0][8] == "奈良旅行"


def test_active_and_archived_project_notes_are_both_listed_symmetrically(tmp_path):
    write_note_in(tmp_path, "進行中案件", "進行中案件.md", "---\ntype: project\nstatus: 1_active\n---\n")
    archive = tmp_path / "3_Archive" / "完了案件"
    archive.mkdir(parents=True)
    (archive / "完了案件.md").write_text("---\ntype: project\nstatus: 2_done\n---\n", encoding="utf-8")

    result = rows(run_index(tmp_path, "--all"))

    assert sorted(row[0] for row in result) == [
        "2_Notes/進行中案件/進行中案件.md",
        "3_Archive/完了案件/完了案件.md",
    ]
