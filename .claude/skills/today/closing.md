...
1. `Inbox/`に項目が残っていれば、各項目を`Cabinet/Templates/review.md`を
   ...
   分類するか（type・context）」を書く。この文章はテンプレートの`- 回答: `
   行より上に書き、`- 回答: `行の内容は空のまま残す（人間がこの行に回答を
   書き込む）。タイトルはWindows/NTFSの禁止文字（`:`等）を避け、
...
2. 今日の`Today.md`の内容（`## ブリーフィング`・`## メモ`）と、今日
   ...
   判断）。後者は、`Cabinet/Notes/`配下で`## 更新履歴`に今日の日付で
   始まる行（`- <今日の日付>:`）を持つノートを抽出して集める。該当が
   あれば`Cabinet/MEMORY.md`に`- <内容>（YYYY-MM-DD記録）`の書式で1行
   追記する（無ければ何もしない）。`Cabinet/MEMORY.md`が存在しなければ
   見出し付きで新規作成する。
