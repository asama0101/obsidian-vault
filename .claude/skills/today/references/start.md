...
6. **カレンダーイベントに対応する議事録ノートを作る。**
   ...
   - **案件を解決する。** `index.py --type project` の結果に対し、会議名とproject名が部分一致するもの（会議名がproject名を含む、またはproject名が会議名を含む）を候補とする（`references/project-management.md` のあいまい一致判定と同水準）。
     ...
     - 候補が2件以上、または0件 → `--project` を付けず `1_Inbox/` に作る。候補が2件以上なら `## Claudeからの連絡` の `### 確認したいこと` に「`<会議名>` の案件はどれですか？候補: `<候補1>`・`<候補2>`…」の1行を、候補が0件なら「`<会議名>` に一致する既存案件がありません。新規作成しますか？（種別タグも教えてください）」の1行を追加する。回答の処理は更新モードのステップ10が行う。
...
8. `## 今日の予定` と `## 今日のタスク` を書く。`## 今日の予定` はカレンダー取得結果から（`references/calendar.md` 参照）、`## 今日のタスク` はタスク索引から（`references/tasks-section.md` 参照）、それぞれAIが全置換で生成する。`## プロジェクト` は `6_Cabinet/Bases/プロジェクト.base` の埋め込みで、テンプレートコピー時に既に反映されているため触らない。
