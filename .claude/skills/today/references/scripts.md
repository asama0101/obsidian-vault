...
- `index.py`: ノートの索引をTSVで出力する。`1_Inbox/`・`2_Notes/`・`3_Archive/`・`4_Knowledge/` の一覧を読む唯一の経路。`rglob` で案件フォルダも走査し、project ノート本体も含めて拾う。既定で完了物（`task` の `4_done`/`5_cancelled`、`project` の `2_done`）とカレンダーID列を落とす。
- `new_note.py`: テンプレートからノートを1件作る。`--type project` は `--project` を無視し、`2_Notes/<title>/<title>.md` を作ると同時に同じフォルダに meeting/task/documents サブフォルダを作る。`--type task`/`meeting` は `--project` を指定すると `2_Notes/<project>/task/` または `2_Notes/<project>/meeting/` に配置する（`project` は既存フォルダ名をそのまま使う文字列で、曖昧一致や自動検索は行わない）。`--project` を指定しない場合、`--type task`/`meeting` は `1_Inbox/` に、`--type know-how` は `4_Knowledge/` に作る。
...
# --with-calendar-ids を付けると末尾に calendar_event_id calendar_series_id が増える（13列）
...
# 今日のタスクチェックリスト（今日やること）生成用
python3 .claude/scripts/index.py --type task

# ノート生成（vault相対パスを標準出力に返す。既存なら終了コード1）
# --project を付けると 2_Notes/<案件名>/ の下に作り、フォルダが無ければ作る
