...
ステップは9つある。うち3つ（1・7・9）はAIだけで完結し、残り6つ（2・3・4・5・6・8）は人間の判断を要する。**6項目を1つずつ順に聞くと締めが6往復になり、毎日実行するのは現実的でない。まとめて1回で聞く。**
...
**`close_day.sh`（ステップ9）は必ず最後に置く。** `close_day.sh` は成功すると `git checkout main` → `git merge --ff-only` → `git push` を行い、**HEAD を `main` に残して終了する。** branch-guard フックは `main`/`master` 上での Write/Edit/MultiEdit、および `mv`/`cp`/`touch`/`sed -i`/`git commit` 等のファイル変更系 Bash コマンドを無条件でブロックする。ステップ2〜8はいずれも frontmatter の Edit・`mv`・`.base` の Edit を伴うため、`close_day.sh` より後に置くと `main` 上でブロックされ遂行不能になる。ファイルを変更するステップは必ず日次ブランチ上で終わらせ、`close_day.sh` の `git add -A` にまとめて拾わせる。
...
6. **`Documents/` 直下を片付ける。** 更新モードで判定しきれなかったファイルの行き先を確認し、直下を空にする。行き先が決まらないものは `Documents/未分類/` へ落として直下を空にし、未分類の件数をサマリーで報告する。フォルダが無ければ `mkdir -p Documents/未分類` で作ってから `mv` する。**`close_day.sh` にこの検査は入れない。**資料の未振り分けはデータ破壊ではなく、ここで締めを止めると急いでいる日に締め自体を放棄することになるため。
