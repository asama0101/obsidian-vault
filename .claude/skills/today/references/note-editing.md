...
- project の `status` を `2_done` にするときは、`2_Notes/<案件名>/` フォルダごと `3_Archive/<案件名>/` へ移動する。
...
- `blocked_by` の変更は frontmatter を直接編集する。阻害要因・レビュー待ちなど、タスクが停滞している理由を人が自由に書くメモ欄で、他タスクへのリンクではない。
- `## 更新履歴` は作らない。
